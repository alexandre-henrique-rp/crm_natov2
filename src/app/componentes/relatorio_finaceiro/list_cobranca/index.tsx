...
export default function ListCobranca() {
  ...
  const lista = Data.data && Data.data.map((item: any) => {
    return (
      <>
        <Flex w={"100%"} flexWrap={"wrap"} py={3} gap={4} px={2} border={"1px solid #000"} borderRadius={"8px"}>
          {/* <Text> id: {item.id}</Text> */}
          <Text>Protocolo: {item.protocolo}</Text>
          <Text>Expedido em: {item.createdAt.split('T')[0].split('-').reverse().join('/')}</Text>
      ...
      {session?.user?.hierarquia === "ADM" ? (
        ...
        `}
          ...
          <Box w={'full'} h={"20vh"} p={5} rounded="lg" boxShadow="2xl">
            ...
            <Flex w={"100%"} h={"100%"} overflowY={"auto"} p={3} mt={2} gap={2} flexDir={"column"}>
