...
import { FiltroComponent } from "@/components/filter/filtro_geral";
import { TabelaDireto } from "@/components/tabela_direto";
...
export default function DiretoPage()  {
  ...
  const [Dados, setDados] = useState<solictacao.SolicitacaoDiretotypes[]>(
    [] as solictacao.SolicitacaoDiretotypes[]
  );
  ...
  useEffect(() => {
    (async () => {
       const req = await fetch ("/api/direto/getall", {
        method: "GET",
        headers: {
