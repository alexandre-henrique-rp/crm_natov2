...
import { Checkbox, Divider, Flex, FormLabel, Heading, Text } from "@chakra-ui/react";
const lista: { name: keyof UserRoler | string; label: string; tag: string }[] = [
  ...
  { name: "cad_construtora", label: "Pode criar Construtora", tag: "construtora" },
  { name: "cad_empreendimento", label: "Pode criar Empreendimento", tag: "empreendimento" },
  ...
  { name: "solicitacao", label: "Pode editar Solicitação", tag: "solicitacao" },
];
...
export default function Permissoes({ data }: Props) {
  return (
    <>
      {!data && <Text>Nenhuma permissão encontrada</Text>}
      {data && (
        <>
